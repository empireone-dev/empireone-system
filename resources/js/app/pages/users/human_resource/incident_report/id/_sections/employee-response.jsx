...
export default function EmployeeResponseModal({ isOpen, onClose, irId }) {
    ...
    const handleSubmit = async () => {
        if (!formData.file) {
            SwalAlert({ icon: "error", title: "Validation Error", text: "Please upload employee response file" });
            return;
        }
        ...
        try {
            ...
            SwalAlert({ icon: "success", title: "Success", text: "Employee response uploaded successfully" });
            setFormData({ notes: "", file: null });
            onClose();
        } catch (error) {
            SwalAlert({ icon: "error", title: "Error", text: "Failed to upload employee response" });
        } finally {
            setLoading(false);
        }
    };
